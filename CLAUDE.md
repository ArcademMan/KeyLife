...
- [x] **Secret key generata al primo avvio** *(ritirata in 0.3.1)*
  `backend/app/core/config.py` — `get_settings()` generava un token
  `secrets.token_urlsafe(32)` su `backend/.secret_key` (mode 0o600, O_EXCL)
  se assente. **Dead code**: nessuna feature ha mai consumato il valore
  (era un anticipo per session signing / HMAC export mai atterrato).
  In 0.3.1 il campo `secret_key` è rimosso da `Settings` e il file viene
  cancellato all'avvio se presente.
...
**Slot keychain unificato (0.3.1)**: dev e build frozen condividono lo
stesso `data_dir` (e quindi lo stesso DB) ⇒ devono usare la stessa
chiave, slot `KeyLife/db`. Lo split `KeyLife-dev/db` introdotto in 0.3.0
era una svista (causava HMAC fail al passaggio dev↔frozen). Resta il
fallback one-shot in `_load_db_key`: se trova il vecchio slot lo migra
a quello canonico e cancella l'originale.
