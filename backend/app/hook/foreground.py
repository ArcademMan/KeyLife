...
class ForegroundHook:
    ...
    def __init__(self, on_change: OnChangeCallback | None = None) -> None:
        ...
        # HWND associato all'ultima risoluzione cache. Confrontato in
        # current() con GetForegroundWindow() per rilevare cambi di focus
        # che SetWinEventHook ha mancato (sintomo: giochi in fullscreen
        # esclusivo come osu! che non emettono EVENT_SYSTEM_FOREGROUND ai
        # consumer out-of-context). Letto/scritto sotto GIL: integer
        # assignment è atomico, niente lock per il check sul hot path.
    ...
    def current(self) -> tuple[str, str | None]:
        ...
        Hot path: chiamata da Aggregator.record() su ogni DOWN. Fa un
        controllo sincrono di `GetForegroundWindow()` per rilevare cambi
        di focus che il hook async ha mancato. È un caso documentato per
        DirectX exclusive fullscreen e alcune app anti-cheat-protette
        (osu!, dove il fullscreen non genera EVENT_SYSTEM_FOREGROUND ai
        listener out-of-context).

        Costo: ~5µs sul fast path (HWND invariato), ~50-100µs sul slow
        path (HWND cambiato → resolve completo). Trascurabile anche a
        regimi di typing molto alti tipo osu! mania.
        """
        try:
            hwnd_now = _user32.GetForegroundWindow()
        except Exception:
            hwnd_now = 0
        if hwnd_now != self._last_hwnd:
            # Slow path: HWND cambiato (o callback async non arrivato).
            # `_handle_foreground_change` aggiorna self._exe/_path/_last_hwnd
            # sotto lock e fa fire del callback `on_change`.
            self._handle_foreground_change(hwnd_now)
        with self._lock:
    ...
    def _run(self) -> None:
        try:
            ...
            try:
                hwnd0 = _user32.GetForegroundWindow()
            except Exception:
                hwnd0 = 0
            self._handle_foreground_change(hwnd0)
    ...
    def _handle_foreground_change(self, hwnd: int) -> None:
        if not hwnd:
            self._update(hwnd, BUCKET_SYSTEM, None)
            return
        pid = wintypes.DWORD(0)
        _user32.GetWindowThreadProcessId(hwnd, ctypes.byref(pid))
        if not pid.value:
        ...
        path = _exe_for_pid(pid.value)
        if not path:
            self._update(hwnd, BUCKET_UNKNOWN, None)
            return
        self._update(hwnd, os.path.basename(path).lower(), path)

    def _update(self, hwnd: int, exe: str, path: str | None) -> None:
        # `hwnd` viene tracciato insieme a (exe, path) sotto lo stesso lock
        # così il fast path di current() non rischia di vedere _last_hwnd
        # già aggiornato a un nuovo HWND mentre _exe/_path puntano ancora al
        # vecchio (race fra keystroke thread e callback async del hook).
        fire_change = False
        with self._lock:
            self._last_hwnd = hwnd
            if exe != self._exe or path != self._path:
                self._exe = exe
                self._path = path
                fire_change = True
        if fire_change and self._on_change is not None:
            try:
