...
def total_keystrokes_range(session: Session, start: str, end: str) -> int:
    """Totale globale di press in [start, end] dalla tabella always-on."""
    stmt = select(func.coalesce(func.sum(DailyKeyCount.count), 0)).where(
        DailyKeyCount.date >= start, DailyKeyCount.date <= end,
    )
    return int(session.execute(stmt).scalar_one())


def total_attributed_range(session: Session, start: str, end: str) -> int:
    """Totale di press attribuiti a una qualunque app in [start, end].

    La differenza con `total_keystrokes_range` rappresenta i tasti
    registrati senza attribuzione: feature off, periodi pre-feature,
    blocklist, oppure foreground non risolvibile (sentinel "unknown"/
    "system" sono attribuzioni valide e quindi *contate* qui).
    """
    stmt = select(func.coalesce(func.sum(DailyAppCount.count), 0)).where(
        DailyAppCount.date >= start, DailyAppCount.date <= end,
    )
    return int(session.execute(stmt).scalar_one())


def unattributed_hourly_range(
    session: Session, start: str, end: str,
) -> list[tuple[str, int, int]]:
    """(date, hour, delta) per il bucket sintetico `<no app>`.

    Per ogni (date, hour) della finestra: delta = hourly_totals.total -
    sum(hourly_app_totals.count). Restituisce solo le celle con delta>0.
    Usato dall'API quando il frontend chiede /apps/hourly?exe_name=<no app>
    per popolare la heatmap-strip oraria del bucket.
    """
    attributed_sum = func.coalesce(func.sum(HourlyAppTotal.count), 0).label("attr")
    attributed_subq = (
        select(HourlyAppTotal.date, HourlyAppTotal.hour, attributed_sum)
        .where(HourlyAppTotal.date >= start, HourlyAppTotal.date <= end)
        .group_by(HourlyAppTotal.date, HourlyAppTotal.hour)
        .subquery()
    )
    delta = (HourlyTotal.total - func.coalesce(attributed_subq.c.attr, 0)).label("delta")
    stmt = (
        select(HourlyTotal.date, HourlyTotal.hour, delta)
        .outerjoin(
            attributed_subq,
            (HourlyTotal.date == attributed_subq.c.date)
            & (HourlyTotal.hour == attributed_subq.c.hour),
        )
        .where(HourlyTotal.date >= start, HourlyTotal.date <= end)
        .order_by(HourlyTotal.date, HourlyTotal.hour)
    )
    return [
        (str(d), int(h), int(c))
        for d, h, c in session.execute(stmt).all()
        if int(c) > 0
    ]
